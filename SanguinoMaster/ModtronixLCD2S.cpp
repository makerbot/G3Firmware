...
static void i2c_write(uint8_t address, uint8_t cmd, uint8_t* data, uint8_t len)
{
  uint8_t buf[len+1];
  buf[0] = cmd;
  memcpy(buf+1, data, len);
  twi_write(address, buf, len+1, 0, NULL);
}



static void i2c_write_P(uint8_t address, uint8_t cmd, PGM_P data, uint8_t len)
{
  uint8_t buf[len+1];
  buf[0] = cmd;
...
{
  twi_write(address, &cmd, 1, 0, NULL);
  return twi_read(address, data, expectedlen);
}



static int i2c_cmd_read_async(uint8_t address, uint8_t cmd, uint8_t expectedlen, twi_callback_t cb)
{
  twi_write(address, &cmd, 1, 0, NULL);
  return twi_read_async(address, expectedlen, cb);
}
...
uint8_t ModtronixLCD2S::read_status()
{
  uint8_t val = 0;
  i2c_cmd_read(address, 0xD0, &val, 1);
  return val;
}



void ModtronixLCD2S::read_status_async(twi_callback_t cb)
{
...
void ModtronixLCD2S::read_keypad_data_async(twi_callback_t cb)
{
...
void ModtronixLCD2S::read_gpio123_async(twi_callback_t cb)
{
